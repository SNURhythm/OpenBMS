...
bool decodeAudioToPCM(const path_t &filePath,
  ...
  sf_count_t numFrames = sf_readf_double(file, tempBuffer.data(), fileInfo.frames);
  if (numFrames < 0) {
    SDL_Log("Failed to read audio data from file %s, error: %s", filePath.c_str(),
            sf_strerror(file));
    return false;
  }
  // Convert the double buffer to short
  std::transform(tempBuffer.begin(), tempBuffer.end(), buffer.begin(),
                 [](double val) { return static_cast<short>(std::clamp(val, -1.0, 1.0) * 32767); });
