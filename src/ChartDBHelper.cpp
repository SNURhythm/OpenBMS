...
bool ChartDBHelper::InsertChartMeta(sqlite3 *db, bms_parser::ChartMeta &chartMeta)
{
    ...
    sqlite3_bind_text(stmt, 1, ToRelativePath(path).string().c_str(), -1, SQLITE_TRANSIENT);
    ...
    sqlite3_bind_text(stmt, 4, ws2s(chartMeta.Title).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 5, ws2s(chartMeta.SubTitle).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 6, ws2s(chartMeta.Genre).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 7, ws2s(chartMeta.Artist).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 8, ws2s(chartMeta.SubArtist).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 9, ws2s_utf8(chartMeta.Folder).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 10, ws2s_utf8(chartMeta.StageFile).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 11, ws2s_utf8(chartMeta.Banner).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 12, ws2s_utf8(chartMeta.BackBmp).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 13, ws2s_utf8(chartMeta.Preview).c_str(), -1, SQLITE_TRANSIENT);
...
void ChartDBHelper::SelectAllChartMeta(sqlite3 *db, std::vector<bms_parser::ChartMeta> &chartMetas)
{
    auto query = "SELECT "
                 "path,"
                 "md5,"
...
void ChartDBHelper::SearchChartMeta(sqlite3 *db, std::string &text, std::vector<bms_parser::ChartMeta> &chartMetas)
{
    auto query = "SELECT "
                 "path,"
                 "md5,"
...
bool ChartDBHelper::DeleteChartMeta(sqlite3 *db, std::filesystem::path &path)
{
    ...
    if (rc != SQLITE_OK)
    {
        std::cerr << "SQL error while preparing statement to delete a chart: " << sqlite3_errmsg(db) << "\n";
        sqlite3_free(stmt);
        return false;
    }
...
bms_parser::ChartMeta ChartDBHelper::ReadChartMeta(sqlite3_stmt *stmt)
{
    ...
    chartMeta.SubTitle = std::wstring(reinterpret_cast<const wchar_t *>(sqlite3_column_text(stmt, idx++)));
    chartMeta.Genre = std::wstring(reinterpret_cast<const wchar_t *>(sqlite3_column_text(stmt, idx++)));
...
bool ChartDBHelper::CreateEntriesTable(sqlite3 *db)
{
    ...
    if (rc != SQLITE_OK)
    {
        std::cerr << "SQL error while creating entries table: " << sqlite3_errmsg(db) << "\n";
        sqlite3_free(errMsg);
        return false;
    }
    return true;
...
bool ChartDBHelper::InsertEntry(sqlite3 *db, std::filesystem::path &path)
{
    ...
    if (rc != SQLITE_OK)
    {
        std::cerr << "SQL error while preparing statement to insert an entry: " << sqlite3_errmsg(db) << "\n";
        sqlite3_close(db);
        return false;
    }
...
std::vector<std::wstring> ChartDBHelper::SelectAllEntries(sqlite3 *db)
{
    auto query = "SELECT "
                 "path"
                 " FROM entries";
...
bool ChartDBHelper::DeleteEntry(sqlite3 *db, std::filesystem::path &path)
{
    ...
    if (rc != SQLITE_OK)
    {
        std::cerr << "SQL error while preparing statement to delete an entry: " << sqlite3_errmsg(db) << "\n";
        sqlite3_free(stmt);
        return false;
    }
    sqlite3_bind_text(stmt, 1, path.string().c_str(), -1, SQLITE_TRANSIENT);
...
std::filesystem::path ChartDBHelper::ToRelativePath(std::filesystem::path &path)
{
...
std::filesystem::path ChartDBHelper::ToAbsolutePath(std::filesystem::path &path)
{
