...
public:
	...
	void SelectAllChartMeta(sqlite3* db, std::vector<bms_parser::ChartMeta>& chartMetas);
	void SearchChartMeta(sqlite3* db, std::string& keyword, std::vector<bms_parser::ChartMeta>& chartMetas);
	...
	std::vector<std::wstring> SelectAllEntries(sqlite3* db);
